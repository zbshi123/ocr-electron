...
class JsondbService extends Service {
  ...
  /*
   * 增 Image data
   * image = {"tag": 0, "name": "", "image_url": "...", ...}
   */
  async addImageData(image) {
    const key = this.demoDBKey.image_data;
  ...
  * 删 Image data
  */
  async delImageDataByImageUrl(image_url = "") {
    const key = this.demoDBKey.image_data;
    const data = this.demoDB.db
    .get(key)
    .remove({image_url: image_url})
    .write();
